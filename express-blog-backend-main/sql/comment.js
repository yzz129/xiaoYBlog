module.exports = {
    ...
    // 获取留言和回复的总数
    GetMsgsTotal: 'SELECT COUNT(*) AS total FROM comments WHERE article_id IS NULL AND approved = 1; SELECT COUNT(*) AS total FROM reply WHERE article_id IS NULL AND approved = 1;',
    // 根据文章ID和用户ID查询评论
    GetCommentsByArticleIDAndUserID: 'SELECT SQL_CALC_FOUND_ROWS id, article_id, content, create_time, nick_name, site_url, avatar, device FROM comments WHERE article_id = ? AND author_id = ? AND deleted = 0 ORDER BY create_time DESC LIMIT ?, ?; SELECT FOUND_ROWS() AS total;',
    // 根据用户ID查询留言
    GetMessagesByUserID: 'SELECT SQL_CALC_FOUND_ROWS id, article_id, content, create_time, nick_name, site_url, avatar, device FROM comments WHERE article_id IS NULL AND author_id = ? AND deleted = 0 ORDER BY create_time DESC LIMIT ?, ?; SELECT FOUND_ROWS() AS total;'
}